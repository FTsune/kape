...
from modules.detection_utils import check_config_changed, get_cache_key, run_detection
from modules.cache_management import update_cache_entry, preload_adjacent_images
...
from components.ui.instructions import (
    top_bar,
    instructions_header,
    subtitle,
    step_item,
...
def get_random_solution(disease_name, disease_data):
    ...
    for disease in disease_data:
        if (
            disease["title"].lower() == disease_name.lower()
            or disease["name2"].lower() == disease_name.lower()
        ):
    ...
    for disease in disease_data:
        if (
            disease_name.lower() in disease["title"].lower()
            or disease_name.lower() in disease["name2"].lower()
        ):
            if disease.get("solution") and len(disease["solution"]) > 0:
                return random.choice(disease["solution"])
...
def render_instructions(
    ...
    for i, step in enumerate(steps, 1):
        st.markdown(
            step_item(i, step, primary_color, text_color), unsafe_allow_html=True
        )
...
def render_results(
    ...
    with stylable_container(
            ...
            with st.container(border=True):
                ...
                if source_img is None:
                    ...
                    if cache_key in st.session_state.image_detection_cache:
                        ...
                        st.session_state["detected_diseases"] = cached_data[
                            "detected_diseases"
                        ]
                        st.session_state["all_disease_detections"] = cached_data[
                            "all_disease_detections"
                        ]
                        st.session_state["disease_confidences"] = cached_data[
                            "disease_confidences"
                        ]
                        st.session_state["all_disease_instances"] = cached_data[
                            "all_disease_instances"
                        ]
                        st.session_state["last_result_image"] = cached_data[
                            "result_image"
                        ]
                        st.session_state["processing_time"] = cached_data.get(
                            "processing_time", 0
                        )
                        ...
                        if config_changed and st.session_state.detection_run:
                            redetect_col1, redetect_col2, redetect_col3 = st.columns(
                                [1, 2, 1]
                            )
                            with redetect_col2:
                                if st.button(
                                    "🔄 Reanalyze objects", use_container_width=True
                                ):
                        ...
                        if st.session_state.get("last_result_image") is not None:
                            ...
                            if (
                                st.session_state.get("last_result_image") is not None
                                and st.session_state.detection_run
                            ):
                                col2_placeholder.image(
                                    st.session_state["last_result_image"],
                                    caption="Detected Image",
                                ...
                                if not st.session_state.detection_run:
                                    ...
                                    try:
                                        ...
                                        if (
                                            results
                                            and results.get("result_image") is not None
                                        ):
                                            # Store results in session state
                                            st.session_state["detected_diseases"] = (
                                                results.get("detected_diseases", [])
                                            )
                                            st.session_state[
                                                "all_disease_detections"
                                            ] = results.get(
                                            ...
                                            st.session_state["disease_confidences"] = (
                                                results.get("disease_confidences", {})
                                            )
                                            st.session_state[
                                                "all_disease_instances"
                                            ] = results.get("all_disease_instances", [])
                                            st.session_state["last_result_image"] = (
                                        ...
                                        st.error(
                                            f"An error occurred during detection: {str(e)}"
                                        )
                                    finally:
            ...
            with st.container():
                ...
                if new_idx != selected_idx:
                    ...
                    preload_adjacent_images(
                        uploaded_images, new_idx, current_model_config, run_detection
                    )
        ...
        st.markdown(
            f"""<h3 style='background-color: {primary_color}10; border-radius: 10px 10px 0px 0px; 
        ...
        with stylable_container(
            ...
            menu_styles = {
                "container": {
                    "padding": "6px",
                    "background-color": f"{primary_color}10",
                },
            ...
            if tab == "Summary":
                if source_img is None:
                    ...
                    disease_confidences = st.session_state.get(
                        "disease_confidences", {}
                    )
                    ...
                    with st.container(border=True):
                        ...
                        st.markdown(
                            '<div style="margin: 10px;></div>"', unsafe_allow_html=True
                        )
                    ...
                    if detected_diseases:
                        ...
                        all_instances = st.session_state.get(
                            "all_disease_instances", []
                        )
                        ...
                        leaf_instances = [
                            ...
                            if inst[0].lower() in leaf_types
                        ]
                        disease_instances = [
                            inst
                            for inst in all_instances
                            if inst[0].lower() not in leaf_types
                        ]

                        # Check if only leaf types or healthy leaves are detected
                        only_leaf_types = all(
                            disease.lower() in leaf_types
                            for disease in detected_diseases
                        )
                        only_healthy = (
                            len(detected_diseases) == 1
                            and detected_diseases[0].lower() == "healthy"
                        )
                        only_healthy_and_leaves = all(
                            disease.lower() == "healthy"
                            or disease.lower() in leaf_types
                            for disease in detected_diseases
                        )
                        ...
                        # For database only:
                        db_buttons_disabled = (
                            current_model_config.get("detection_model_choice", "")
                            == "Leaf"  # Leaf model only
                            or only_leaf_types  # Only leaf types detected
                            or only_healthy  # Only healthy leaves
                            or only_healthy_and_leaves  # Only healthy leaves + leaf types
                        )
                        ...
                        with col1:
                            ...
                            db_button_disabled = (
                                st.session_state.saved_to_database
                                or db_buttons_disabled
                            )

                            if (
                                db_buttons_disabled
                                and not st.session_state.saved_to_database
                            ):
                                if (
                                    current_model_config.get(
                                        "detection_model_choice", ""
                                    )
                                    == "Leaf"
                                ):
                                    st.info(
                                        "Coffee leaf types are not saved to database",
                                        icon=":material/info:",
                                    )
                                    ...
                                    st.info(
                                        "Healthy leaves are not saved to database",
                                        icon=":material/info:",
                                    )
                                elif only_healthy_and_leaves:
                                    st.info(
                                        "Healthy leaves and leaf types are not saved to database",
                                        icon=":material/info:",
                                    )
                            else:
                                if st.button(
                                    ...
                                    try:
                                        ...
                                        with st.spinner("Saving to database..."):
                                            ...
                                            for disease in saveable_diseases:
                                                confidence = disease_confidences.get(
                                                    disease, 0
                                                )
                                                if confidence >= 60:
                                                    high_confidence_diseases.append(
                                                        (disease, confidence)
                                                    )

                                            if not high_confidence_diseases:
                                                st.warning(
                                                    "Confidence score is too low. Try another image."
                                                )
                                            else:
                                                for (
                                                    disease,
                                                    confidence,
                                                ) in high_confidence_diseases:
                                                    save_location_data(
                                                        source_img,
                                                        disease,
                                                        confidence,
                                ...
                                if (
                                    db_button_disabled
                                    and st.session_state.saved_to_database
                                ):
                        ...
                        with col2:
                            ...
                            if source_img:
                                try:
                                    image_exists = check_image_exists(
                                        drive, PARENT_FOLDER_ID, source_img.name
                                    )
                                except:
                            ...
                            drive_button_disabled = (
                                st.session_state.saved_to_drive
                                or image_exists
                                or drive_buttons_disabled
                            )

                            if (
                                drive_buttons_disabled
                                and not st.session_state.saved_to_drive
                            ):
                                if (
                                    current_model_config.get(
                                        "detection_model_choice", ""
                                    ...
                                    st.info(
                                        "Coffee leaf types are not saved to Drive",
                                        icon=":material/info:",
                                    )
                                elif only_leaf_types:
                                    st.info(
                                ...
                                if st.button(
                                    ...
                                    try:
                                        ...
                                        if check_image_exists(
                                            ...
                                            st.warning(
                                                "⚠️ This image already exists in Google Drive"
                                            )
                                        else:
                                            ...
                                            if not saveable_diseases:
                                                ...
                                                for (
                                                    ...
                                                    if (
                                                        disease
                                                        not in highest_confidence
                                                        or confidence
                                                        > highest_confidence[disease]
                                                    ):
                                                        highest_confidence[disease] = (
                                                            confidence
                                                        )
                                                ...
                                                if not high_confidence_diseases:
                                                    ...
                                                    for (
                                                        ...
                                                        high_confidence_diseases.items()
                                                    ):
                                                    ...
                                                    st.success(
                                                        f"✅ Image uploaded to {success_count} disease folders in Google Drive!"
                                                    )
                                    except Exception as e:
                                ...
                                if (
                                    drive_button_disabled
                                    and st.session_state.saved_to_drive
                                ):
                ...
                if source_img is None:
                    ...
                    if model_type in ["Leaf", "Both Models"]:
                        ...
                        leaf_instances = [
                            inst
                            for inst in all_instances
                            if inst[0].lower() in ["arabica", "liberica", "robusta"]
                        ]
                        ...
                        if leaf_instances:
                            with st.expander("Leaf Types"):
                                for leaf_type, score in leaf_instances:
                                    st.markdown(
                                        f"- **{leaf_type.title()}**: {score:.1f}% confidence"
                                    )
                        else:
                            with st.container():
                    ...
                    st.markdown(
                        f"""<div style="margin: 10px 15px 10px 5px; font-size: 20px; font-weight: 600; color: {primary_color}">
                                Image Metadata
                                </div>""",
                        unsafe_allow_html=True,
                    )
                    ...
                    with st.container():
                        if date_taken or location_name:
                            if date_taken:
                                st.markdown(
                                    f"📅 **Date Taken**: {date_taken.strftime('%B %d, %Y')}"
                                )
                            if location_name:
                    ...
                    disease_instances = [
                        disease
                        for disease in detected_diseases
                        if disease.lower() not in leaf_types
                        and disease.lower() != "healthy"
                    ]
                    ...
                    if disease_instances:
                        ...
                        if any(
                            ...
                            with st.container():
                                st.success(
                                    "👍 Your coffee leaf appears healthy! No treatment needed."
                                )
                        else:
                            with st.container():
                                st.info(
                                    "No disease instances requiring recommendations detected in this image"
                                )
                    ...
                    disease_instances = [
                        inst
                        for inst in all_instances
                        if inst[0].lower() not in ["arabica", "liberica", "robusta"]
                    ]
...
def manage_ui_state(theme_colors):
    ...
    if (
        "previous_batch_mode" in st.session_state
        and st.session_state.previous_batch_mode != batch_mode
    ):
